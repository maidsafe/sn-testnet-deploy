...
async fn should_run_ansible_against_genesis_with_a_custom_binary() -> Result<()> {
    let extra_vars_doc = r#"{ "provider": "digital-ocean", "testnet_name": "beta", "node_archive_url": "CUSTOM_BIN_URL", "safenode_rpc_client_archive_url": "CUSTOM_RPC_CLIENT_BIN_URL", "logstash_stack_name": "main", "logstash_hosts": ["10.0.0.1:5044", "10.0.0.2:5044"] }"#;
    let (tmp_dir, working_dir) = setup_working_directory()?;
    let s3_repository = setup_deploy_s3_repository("beta", &working_dir)?;
    let mut ansible_runner = MockAnsibleRunnerInterface::new();
    ansible_runner
