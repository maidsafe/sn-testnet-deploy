...
const PORT_RESTRICTED_CONE_PRIVATE_NODE: &str = "port_restricted_cone_private_node";
const PORT_RESTRICTED_CONE_PRIVATE_NODE_ATTACHED_VOLUME: &str = "port_restricted_cone_private_node_attached_volume";
...
pub struct InfraRunOptions {
    ...
    pub port_restricted_cone_nat_gateway_vm_size: Option<String>,
...
impl InfraRunOptions {
    ...
    pub async fn generate_existing(
        ...
        let port_restricted_cone_private_node_vm_count = resource_count(PORT_RESTRICTED_CONE_PRIVATE_NODE);
        debug!("Port restricted cone private node count: {port_restricted_cone_private_node_vm_count}");
        let (port_restricted_cone_private_node_volume_size, port_restricted_cone_nat_gateway_vm_size) =
            if port_restricted_cone_private_node_vm_count > 0 {
                let port_restricted_cone_private_node_volume_size = get_value_for_resource(
                    &resources,
                    PORT_RESTRICTED_CONE_PRIVATE_NODE_ATTACHED_VOLUME,
                    SIZE,
                )?;
                debug!(
                    "Port restricted cone private node volume size: {port_restricted_cone_private_node_volume_size:?}"
                );
                // gateways should exists if private nodes exist
                let port_restricted_cone_nat_gateway_vm_size =
                    get_value_for_resource(&resources, PORT_RESTRICTED_CONE_NAT_GATEWAY, SIZE)?;

                debug!("Port restricted cone nat gateway size: {port_restricted_cone_nat_gateway_vm_size:?}");
                ...
                (
                    ...
                    port_restricted_cone_nat_gateway_vm_size,
                )
            } else {
                (None, None)
            };
        ...
        let options = Self {
            ...
            port_restricted_cone_nat_gateway_vm_size,
            port_restricted_cone_private_node_vm_count: Some(port_restricted_cone_private_node_vm_count),
            port_restricted_cone_private_node_volume_size,
