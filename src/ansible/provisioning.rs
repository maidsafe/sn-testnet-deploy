...
impl From<BootstrapOptions> for ProvisionOptions {
    fn from(bootstrap_options: BootstrapOptions) -> Self {
        ProvisionOptions {
            ...
            enable_logging: bootstrap_options.enable_logging,
            enable_metrics: true,
            evm_data_payments_address: bootstrap_options.evm_data_payments_address,
            evm_merkle_payments_address: bootstrap_options.evm_merkle_payments_address,
            ...
            network_dashboard_branch: bootstrap_options.network_dashboard_branch,
...
impl From<DeployOptions> for ProvisionOptions {
    fn from(deploy_options: DeployOptions) -> Self {
        ProvisionOptions {
            ...
            disable_nodes: false,
            delayed_verifier_quorum_value: None,
            enable_logging: deploy_options.enable_logging,
            ...
            evm_merkle_payments_address: deploy_options.evm_merkle_payments_address,
...
impl From<ClientsDeployOptions> for ProvisionOptions {
    fn from(client_options: ClientsDeployOptions) -> Self {
        Self {
            ...
            start_performance_verifier: client_options.start_performance_verifier,
            start_uploaders: client_options.start_uploaders,
            evm_data_payments_address: client_options.evm_details.data_payments_address,
            evm_merkle_payments_address: client_options.evm_details.merkle_payments_address,
            ...
            data_retrieval_service_count: client_options.data_retrieval_service_count,
...
impl AnsibleProvisioner {
    ...
    pub async fn provision_static_uploader(
        ...
        let sk_map = if let Some(wallet_keys) = &options.wallet_secret_keys {
            ...
            self.deposit_funds_to_clients(&FundingOptions {
                ...
                evm_merkle_payments_address: options.evm_merkle_payments_address.clone(),
    ...
    pub fn apply_delete_node_records_cron(
        ...
        if let Some(node_type) = node_type {
            println!("Applying delete node records cron for {node_type:?} nodes");
            self.ansible_runner.run_playbook(
        ...
        if let Some(custom_inventory) = custom_inventory {
            println!("Applying delete node records cron with a custom inventory");
            generate_custom_environment_inventory(
                &custom_inventory,
                environment_name,
                &self.ansible_runner.working_directory_path.join("inventory"),
        ...
        println!("Applying delete node records cron for all node types");
        for node_inv_type in AnsibleInventoryType::iter_node_type() {
            self.ansible_runner.run_playbook(
                AnsiblePlaybook::ApplyDeleteNodeRecordsCron,
                node_inv_type,
