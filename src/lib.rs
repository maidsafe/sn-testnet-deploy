...
mod test {
    ...
    #[tokio::test]
    async fn deploy_should_run_terraform_apply() -> Result<()> {
        let (tmp_dir, working_dir) = setup_working_directory()?;
        let s3_repository = setup_default_s3_repository(&working_dir)?;
        let mut terraform_runner = MockTerraformRunnerInterface::new();
        terraform_runner
    ...
    #[tokio::test]
    async fn deploy_should_run_terraform_apply_with_custom_bin_set_when_repo_is_supplied(
    ) -> Result<()> {
        let (tmp_dir, working_dir) = setup_working_directory()?;
        let s3_repository = setup_default_s3_repository(&working_dir)?;
        let mut terraform_runner = MockTerraformRunnerInterface::new();
    ...
    #[tokio::test]
    async fn deploy_should_ensure_branch_is_set_if_repo_owner_is_used() -> Result<()> {
        let (tmp_dir, working_dir) = setup_working_directory()?;
        let s3_repository = setup_default_s3_repository(&working_dir)?;
        let mut terraform_runner = MockTerraformRunnerInterface::new();
        terraform_runner
    ...
    #[tokio::test]
    async fn deploy_should_ensure_repo_owner_is_set_if_branch_is_used() -> Result<()> {
        let (tmp_dir, working_dir) = setup_working_directory()?;
        let s3_repository = setup_default_s3_repository(&working_dir)?;
        let mut terraform_runner = MockTerraformRunnerInterface::new();
        terraform_runner
